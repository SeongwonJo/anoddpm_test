...
def make_videos():
    ...
    elif args["dataset"].lower() == "nfbs":
        d_set = dataset.MRIDataset_custom("../dataset/BraTS/data/MICCAI_BraTS2020_TrainingData/", img_size=args['img_size'], train=False, random_slice=True)
    # else:
    ...
    t_distance_list = [500]
    num_iter = 5   # snu : 1391  /  kaggle : 390    || snuhe_sample : 9   /   kaggle_sample : 5
...
def make_gauss_simplex_outputs(simplex_argNum="28", gauss_argNum="26"):
    ...
    if args_simplex["dataset"].lower() == "carpet":
        ...
        if in_channels == 3:
            d_set = dataset.MVTec(
                    "../data/leather", anomalous=True, img_size=args_simplex["img_size"],
                    rgb=False, include_good=False
                    )
        else:
            d_set = dataset.MVTec(

...
def init_dataset_loader(mri_dataset, args, shuffle=False):
    _sampler = SequentialSampler(mri_dataset)
    dataset_loader = cycle(
            torch.utils.data.DataLoader(
                    mri_dataset,
...
class custom(Dataset):
    def __init__(self, dir, anomalous=False, img_size=(256, 256), rgb=False, e_aug=False, include_good=False,):
        # dir = './DATASETS/leather'
...
if __name__ == "__main__":
    ...
    d_set = MVTec(
            '../data/chest_xray/', img_size=(256, 256), rgb=False
            )
